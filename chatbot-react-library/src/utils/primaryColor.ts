...
/** CSS custom properties for brand color: primary, user bubble, and thinking block. */
export function buildPrimaryColorStyle(
  ...
  const userBubble =
    resolvedTheme === "dark"
      ? rgbToHex(mix(rgb, { r: 0, g: 0, b: 0 }, 0.32))
      : primary;
  const userBubbleHover =
    resolvedTheme === "dark"
      ? rgbToHex(mix(rgb, { r: 0, g: 0, b: 0 }, 0.22))
      : hover;

  const thinkingLabel =
    resolvedTheme === "dark"
      ? rgbToHex(mix(rgb, { r: 255, g: 255, b: 255 }, 0.28))
      : primary;
  const thinkingText =
    resolvedTheme === "dark"
